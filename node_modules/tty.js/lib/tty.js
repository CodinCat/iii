...
Server.prototype.handleConnection = function(socket) {
  ...
  var javahome = '/home/workspace/javahome';
  ...
  socket.on('kill', function(id) {
    var realId = id.split('/')[3];
    var path = javahome +  '/' + realId;
    var javaPath = path + '/Main.java';
    fs.unlinkSync(javaPath);
    fs.unlinkSync(path + '/ans');
    fs.rmdir(path);
    var child = exec('sudo docker stop docker' +  realId, function() {
    var child2 = exec('sudo docker rm docker' + realId);
    });
  ...
  socket.on('run', function(id, data) {
    ...
    exists = fs.existsSync(javaPath);
    if (exists) {
        fs.unlinkSync(javaPath);
    }
    fs.writeFileSync(javaPath, data);
    fs.writeFileSync(path + '/ans', "Hello world!\nPlease input: HelloWorld\nYour input: HelloWorld"); // ans file
    return session.handleData(id, 'sh /home/workspace/rundocker.sh ' + realId + '\nsh /data/run.sh\n\n');
  });
